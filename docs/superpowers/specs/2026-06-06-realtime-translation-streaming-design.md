...
- `TextEmissionPolicy` 负责“下一次文本是否值得发给字幕层”，包括中文单字合并、标点 flush、最终 flush 和改写 flush。
...
- `caption-store` 继续兜底合帧，但主要防线应在 Agent 输出侧。

## 文本发射策略

### 源文 ASR

源文可以比译文更激进，但不能按 CJK 单字更新：

- 英文/拉丁文本按词或短语到达时立即显示。
- 纯 CJK 单字 delta 累积到至少 4 个可见字符再发。
