...
2026-06-07 ElevenLabs 情绪控制修订：同传不应像角色朗读一样有大幅情绪起伏。根据 ElevenLabs voice settings 语义，当前默认把 `ELEVENLABS_STABILITY` 提高到 `0.85`，把 `ELEVENLABS_STYLE` 设为 `0.0`，关闭 `ELEVENLABS_USE_SPEAKER_BOOST`，并保留 `ELEVENLABS_SIMILARITY_BOOST=0.75`。Agent 的 capabilities 极短 TTS 探测和真实 TTS 请求都使用同一组 voice settings，避免预检通过但真实请求参数不同。
